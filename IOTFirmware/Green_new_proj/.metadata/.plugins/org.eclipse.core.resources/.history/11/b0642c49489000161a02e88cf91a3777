...
#define USART_TX_BUFFER_SIZE 41
//#define DEBUG_SYSN
//#define USART_CHECK
//#define DEBUG
//#define DEBUG_USART
...
*/
typedef union {
  char bufferDrvRx[USART_RX_BUFFER_SIZE];
  struct{
	  uint16_t tempVal;
	  uint16_t humdVal;
	  uint16_t ecVal;
...
typedef union {
	char ctrAppData[USART_TX_BUFFER_SIZE];
	 struct
	    {
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  StaticJsonBuffer<768> jsonRxBuffer;
  ...
  appDataRx.sysTAIL   = 255 ; // 11111111
...
{	//int cntt;
	...
	while(Serial.available()){
		...
		*/
		...
		if(seriCnt >= USART_RX_BUFFER_SIZE){
	...
	if( cntStatus == 0){
	  //setup_wifi();
		...
		if(!client.connected()){
			...
			if(client.connect("ESPClient4", userName, userPassword)){
				client.subscribe("ndbn200491_pub");
